...
if __name__ == '__main__':
    ...
    path = os.getcwd()
    Dataset = 'BMNC'
    model_output = path + '/results_preprocessing/{}'.format(Dataset)
    Path = 'D:/Tesedemo/from scEMC/BMNC/'
    ...
    for i in range(omics_df_num):
        single_omics = pd.read_csv(Path + args.data[i], header=0, index_col=0, sep=',').T
        # single_omics.rename(columns={col: col.replace('.', '-') for col in single_omics.columns}, inplace=True)  # for In_house_PBMC2000 列名不同
        single_omics = single_omics.T  # row -> cells   col -> feature
        single_omics = pd.DataFrame(np.nan_to_num(single_omics.values, copy=False).astype(float),
                                    index=single_omics.index.values, columns=single_omics.columns.values)
        if args.scale == "minmax":
            scaler_method = sp.MinMaxScaler()
            init_om_data = scaler_method.fit_transform(single_omics.values)
            init_om_data_df = pd.DataFrame(init_om_data, index=single_omics.index.values,
                                           columns=single_omics.columns.values)
    ...
    init_mo_df.to_csv("./results_preprocessing/{}/1.{}_Multi-Omics_Data.csv".format(Dataset, args.scale))
    label_df = pd.read_csv('D:/Tesedemo/MOINER_label/' + args.label, header=0, index_col=0, sep=',')
    np.save("./results_preprocessing/{}/5.Data_label.npy".format(Dataset), label_df.loc[:, "Label"].values.reshape(-1, 1))
    ...
    fused_network_df = pd.DataFrame(snf.snf(affinity_networks, K=20), index=omics_df_list[0].index.values,
                                    columns=omics_df_list[0].index.values)
    print("   [IE-MOIF] --Info: Try to save fused similarity matrix...")
    fused_network_df.to_csv("./results_preprocessing/{}/2.Sample_similarity_network_matirx.csv".format(Dataset))
    ...
    if args.drm == "fs":
        ...
        for i in range(omics_df_num):
            ...
            omics_df_list[i].iloc[:, feature_index].to_csv(
                "./results_preprocessing/{}/3.{}_feature_selection.csv".format(Dataset, args.type[i]))
        drm_df = pd.concat(fs_mo_df_list, axis=1)
        drm_df.to_csv("./results_preprocessing/{}/3.FS_dimension_reduced_matirx.csv".format(Dataset))
    ...
    snf_fused_mo_df = pd.DataFrame(np.matmul(fused_network_df.values, drm_df.values), index=drm_df.index.values,
                                   columns=drm_df.columns.values)
    print("   [IE-MOIF] --Info: Try to save the sample multi-omics matrix fused with sample-similarity network...")
    snf_fused_mo_df.to_csv("./results_preprocessing/{}/4.Multi-Omics_Data(Information Enhanced).csv".format(Dataset))
